/**
 * @file HTTP 客户端
 * @description 基于 fetch 的 HTTP 客户端，支持请求/响应拦截器、超时控制、自动重试
 */
 ...
 * 拦截器可以：
 * 1. 抛出错误 - 终止请求并向上传播错误
 * 2. 返回恢复值 - 作为请求的成功结果返回（用于 Token 续期后重试）
 * 3. 返回 undefined - 继续执行后续拦截器或抛出原始错误
 */
...
export class HttpClient {
  ...
  private buildUrl(endpoint: string, params?: Record<string, unknown>): string {
    // 规范化 basePath 和 endpoint，确保只有一个斜杠连接
    const normalizedBasePath = this.basePath.endsWith('/') ? this.basePath.slice(0, -1) : this.basePath
    const normalizedEndpoint = endpoint.startsWith('/') ? endpoint : `/${endpoint}`

    // 处理相对路径的 basePath（如 /api），需要拼接 origin
    const fullPath = `${normalizedBasePath}${normalizedEndpoint}`
    const base = fullPath.startsWith('http')
      ? fullPath
      : `${typeof window !== 'undefined' ? window.location.origin : 'http://localhost'}${fullPath}`
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          // 执行错误拦截器，支持恢复机制（如 Token 续期后重试）
          for (const interceptor of this.errorInterceptors) {
            const recovered = await interceptor(httpError, config)
            // 如果拦截器返回了非 undefined/null 值，视为恢复成功，直接返回
